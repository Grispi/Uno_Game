...
});return{...r,...(type==='redirect'?{statusCode:(0,_checkCustomRoutes.getRedirectStatus)(r),permanent:undefined}:{}),regex:routeRegex.source};};const routesManifestPath=_path.default.join(distDir,_constants2.ROUTES_MANIFEST);const routesManifest={version:1,pages404:true,basePath:config.experimental.basePath,redirects:redirects.map(r=>buildCustomRoute(r,'redirect')),rewrites:rewrites.map(r=>buildCustomRoute(r,'rewrite')),headers:headers.map(r=>buildCustomRoute(r,'header')),dynamicRoutes:(0,_utils.getSortedRoutes)(dynamicRoutes).map(page=>({page,regex:(0,_utils.getRouteRegex)(page).re.source}))};await mkdir(distDir,{recursive:true});// We need to write the manifest with rewrites before build
...
try{await fsUnlink(_path.default.join(distDir,'static',buildId,'pages',actualPage+'.js'));}catch(err){if(err.code!=='ENOENT'){throw err;}}}if(result.hasStaticProps){ssgPages.add(page);isSsg=true;if(result.prerenderRoutes){additionalSsgPaths.set(page,result.prerenderRoutes);ssgPageRoutes=result.prerenderRoutes;}if(result.prerenderFallback){hasSsgFallback=true;ssgFallbackPages.add(page);}}else if(result.hasServerProps){serverPropsPages.add(page);}else if(result.isStatic&&customAppGetInitialProps===false){staticPages.add(page);isStatic=true;}if(hasPages404&&page==='/404'){if(!result.isStatic&&!result.hasStaticProps){throw new Error(_constants.PAGES_404_GET_INITIAL_PROPS_ERROR);}// we need to ensure the 404 lambda is present since we use
